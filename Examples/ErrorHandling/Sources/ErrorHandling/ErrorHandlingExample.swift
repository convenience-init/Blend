...
public func demonstrateBasicErrorHandling() async {
...
public func demonstrateHTTPErrorHandling() async {
...
public func demonstrateTimeoutErrorHandling() async {
...
public func demonstrateInvalidURLErrorHandling() async {
    blendLogger.info("Invalid URL Error")
    let service = ErrorSimulationService(errorCase: .invalidEndpoint(reason: "Invalid URL format"))
...
public func demonstrateAuthErrorHandling() async {
    blendLogger.info("Authentication Error")
    let service = ErrorSimulationService(errorCase: .unauthorized(data: nil, statusCode: 401))
...
public func demonstrateDecodingErrorHandling() async {
    blendLogger.info("JSON Decoding Error")
    let decodingError = NSError(
        domain: "Test", code: 1, userInfo: [NSLocalizedDescriptionKey: "Invalid JSON format"])
    let service = ErrorSimulationService(
...
public func demonstrateImageErrorHandling() async {
...
public func demonstrateCacheErrorHandling() async {
...
public func demonstrateTransportErrorHandling() async {
...
public func demonstrateErrorRecovery() async {
    ...
    do {
        // First attempt should fail
        let _: String = try await serverRecoveryService.sendRequest(String.self, to: MockEndpoint())
        blendLogger.warning("Unexpected success on first attempt")
...
public func demonstrateComprehensiveErrorHandling() async {
    ...
    let errorCases: [NetworkError] = [
        ...
        .decodingError(
            underlying: NSError(
                domain: "Test", code: 1, userInfo: [NSLocalizedDescriptionKey: "Invalid JSON"]),
