...
public enum NetworkError: Error, LocalizedError, Sendable, Equatable {
    ...
    private static func statusMessage(_ format: String, _ statusCode: Int) -> String {
        return String(
            format: NSLocalizedString(
                format, comment: "Error message for HTTP errors with status code"), statusCode)
    }
    ...
    private static func transportErrorDescription(_ code: URLError.Code) -> String {
        ...
        case .timedOut:
            return NSLocalizedString(
                "Request timed out", comment: "User-friendly description for URLError.Code.timedOut"
            )
        case .cannotFindHost:
            return NSLocalizedString(
                "Cannot find host",
        ...
        case .badURL:
            return NSLocalizedString(
                "Bad URL", comment: "User-friendly description for URLError.Code.badURL")
        case .unsupportedURL:
            return NSLocalizedString(
                "Unsupported URL",
        ...
        case .serverCertificateHasBadDate:
            return NSLocalizedString(
                "Server certificate has bad date",
        ...
        case .downloadDecodingFailedToComplete:
            return NSLocalizedString(
                "Download decoding failed to complete",
                comment:
        ...
        case .backgroundSessionWasDisconnected:
            return NSLocalizedString(
                "Background session was disconnected",
                comment:
        ...
        default:
            ...
            return String(
                format: NSLocalizedString(
                    "Transport error %d", comment: "Fallback description for unknown URLError codes"
                ), code.rawValue)
        }
    }
    ...
    public var errorDescription: String? {
        ...
        case .decodingFailed(let reason, _, _):
            return String(
                format: NSLocalizedString(
                    "Decoding failed: %@", comment: "Error message for detailed decoding failures"),
                reason)
        case .networkUnavailable:
            return NSLocalizedString(
                "Network unavailable.", comment: "Error message when network is not available")
        case .requestTimeout(let duration):
            return String(
                format: NSLocalizedString(
                    "Request timed out after %.2f seconds.",
        ...
        case .unauthorized(_, let statusCode):
            return String(
                format: NSLocalizedString(
                    "Not authorized (status %d).",
                    comment: "Error message for unauthorized access with HTTP status code"),
                statusCode)
        case .noResponse:
            return NSLocalizedString(
                "No network response.", comment: "Error message when no response is received")
        case .badMimeType(let mimeType):
            return String(
                format: NSLocalizedString(
                    "Unsupported MIME type: %@", comment: "Error message for unsupported MIME types"
                ), mimeType)
        case .uploadFailed(let message):
            return String(
                format: NSLocalizedString(
                    "Upload failed: %@", comment: "Error message for upload failures with details"),
                message)
        case .imageProcessingFailed:
            return NSLocalizedString(
        ...
        case .cacheError(let message):
            return String(
                format: NSLocalizedString(
                    "Cache error: %@", comment: "Error message for cache errors with details"),
                message)
        case .invalidBodyForGET:
            return NSLocalizedString(
        ...
        case .requestCancelled:
            return NSLocalizedString(
                "Request was cancelled.", comment: "Error message for cancelled requests")
        case .authenticationFailed:
            return NSLocalizedString(
                "Authentication failed.", comment: "Error message for authentication failures")
    ...
    public var recoverySuggestion: String? {
        ...
        case .httpError(let statusCode, _):
            ...
            case 404:
                return NSLocalizedString(
                    "Verify the endpoint URL and resource exists.",
            ...
            case 500..<600:
                return NSLocalizedString(
                    "Try again later. The server encountered an error.",
        ...
        case .decodingError:
            return NSLocalizedString(
                "Ensure the response format matches the expected model.",
        ...
        case .requestTimeout:
            return NSLocalizedString(
                "Try again with a better connection or increase timeout duration.",
        ...
        case .badMimeType:
            return NSLocalizedString(
                "Ensure the server returns a supported content format.",
                comment: "Recovery suggestion for unsupported MIME type errors")
        case .uploadFailed:
            return NSLocalizedString(
                "Check content format and network connection.",
                comment: "Recovery suggestion for upload failure errors")
        case .imageProcessingFailed:
            return NSLocalizedString(
                "Ensure the content data is valid and supported.",
                comment: "Recovery suggestion for image processing failure errors")
        case .cacheError:
            return NSLocalizedString(
                "Check cache configuration and available memory.",
        ...
        case .requestCancelled:
            return NSLocalizedString(
                "Check cancellation logic and consider retrying if appropriate.",
                comment:
